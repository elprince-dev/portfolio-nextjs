...
/**
 * Blog API route (preserved, Req 16.1, 16.4).
 *
 * Proxies the GitHub-hosted blog content repository, parsing frontmatter from
 * each markdown file. Individual fetch failures are logged and skipped so the
 * route returns the successfully fetched posts (preserved fallback behavior),
 * and a top-level failure returns a 500 error. Migrated to TypeScript with no
 * behavioral change.
 */
