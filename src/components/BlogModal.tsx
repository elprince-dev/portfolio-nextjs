...
/**
 * BlogModal — preserved blog modal behavior (Req 16.1, 16.4).
 *
 * Renders a selected blog post's markdown content in an animated modal with
 * syntax-highlighted code blocks. Migrated to TypeScript with no behavioral
 * change.
 */
...
const BlogModal: React.FC<BlogModalProps> = ({ blog, isOpen, onClose }) => {
  ...
  const components: Components = {
    ...
    h1: ({ children }) => (
      <h1 className="text-3xl font-semibold mb-8 text-gray-900 dark:text-white leading-tight">
        {children}
      </h1>
    ),
    h2: ({ children }) => (
      <h2 className="text-2xl font-semibold mb-6 mt-12 text-gray-900 dark:text-white leading-tight border-b border-gray-200 dark:border-gray-700 pb-3">
        {children}
      </h2>
    ),
    h3: ({ children }) => (
      <h3 className="text-xl font-semibold mb-4 mt-8 text-gray-900 dark:text-white leading-tight">
        {children}
      </h3>
    ),
    p: ({ children }) => (
